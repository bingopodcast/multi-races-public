...
from multi_races.graphics.long_shot import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.long_shot.display, param=self)

    def sw_flag_active(self, sw):
		if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
			self.game.end_run_loop()
			del self.game.proc
			os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh long_shot")
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.long_shot.display(self)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.long_shot.reel1, graphics.long_shot.reel10, graphics.long_shot.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.long_shot.reel1, graphics.long_shot.reel10, graphics.long_shot.reel100)
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.long_shot.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.long_shot.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.long_shot.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class LongShot(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(LongShot, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "long_shot", "continuous")
...
game = LongShot(machine_type='pdb')
game.reset()
game.run_loop()
