...
from multi_races.graphics.arlington import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.arlington.display, param=self)
	...
	os.system("sshpass -p raspberry ssh pi@10.0.0.52 /home/pi/sc.sh %s &" % ("arlington_ok"))
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.arlington.display(self)
        self.tilt_actions()
        self.replay_step_down(self.game.replays)

    def tilt_actions(self):
	os.system("sshpass -p raspberry ssh pi@10.0.0.52 /home/pi/sc.sh %s &" % ("arlington"))
        self.game.coils.payout.disable()
        self.game.payout_number = 0
        self.game.start.disengage()
	...
	if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
	    ...
	    del self.game.proc
	    os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh arlington")
        ...
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if area == "purse":
                if self.game.replay_counter.position < purse:
                    ...
                    self.winner_replay_step_up(purse, True)
            elif area == "show":
                if self.game.replay_counter.position < show:
                    self.game.search_index.engage(self.game)
                    self.winner_replay_step_up(show, True)
            elif area == "place":
                if self.game.replay_counter.position < place:
                    self.game.search_index.engage(self.game)
                    self.winner_replay_step_up(place, True)
            else:
                if area == "win":
                    if self.game.replay_counter.position < win:
                        self.game.search_index.engage(self.game)
                        self.winner_replay_step_up(win, True)
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.arlington.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.arlington.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Arlington(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(Arlington, self).__init__(machine_type)
...
game = Arlington(machine_type='pdb')
game.reset()
game.run_loop()
