...
from multi_races.graphics.jockey_special import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.jockey_special.display(self)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.jockey_special.reel1, graphics.jockey_special.reel10, graphics.jockey_special.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.jockey_special.reel1, graphics.jockey_special.reel10, graphics.jockey_special.reel100)
        ...
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if area == "feature" and self.game.pennant.status == True:
                if self.game.replay_counter.position < 160:
                    ...
                    self.winner_replay_step_up(160 - self.game.replay_counter.position * m)
            if area == "purse":
                if self.game.replay_counter.position < purse:
                    self.game.search_index.engage(self.game)
                    self.winner_replay_step_up(purse - self.game.replay_counter.position * m)
            elif area == "show":
                if self.game.replay_counter.position < show:
                    self.game.search_index.engage(self.game)
                    self.winner_replay_step_up(show - self.game.replay_counter.position * m)
            elif area == "place":
                if self.game.replay_counter.position < place:
                    self.game.search_index.engage(self.game)
                    self.winner_replay_step_up(place - self.game.replay_counter.position * m)
            else:
                if area == "win":
                    if self.game.replay_counter.position < win:
                        self.game.search_index.engage(self.game)
                        self.winner_replay_step_up(win - self.game.replay_counter.position * m)
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.jockey_special.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.jockey_special.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.jockey_special.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.jockey_special.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def check_reflex(self):
        if self.game.reflex.connected_rivet() == 5:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("jockey_special") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 4 and self.game.mixer1.connected_rivet("jockey_special") not in [2,3,4,5,7,9,10,12,15,17,19,20,21,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("jockey_special") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 3 and self.game.mixer1.connected_rivet("jockey_special") not in [2,5,7,9,12,15,19,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("jockey_special") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 2 and self.game.mixer1.connected_rivet("jockey_special") not in [5,9,12,15,19,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("jockey_special") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 1 and self.game.mixer1.connected_rivet("jockey_special") not in [5,9,15,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("jockey_special") not in [3,10,14,20]:
                return 1
        else:
            return 0
...
class JockeySpecial(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(JockeySpecial, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "jockey_special", "continuous")
        ...
        self.coin = units.Stepper("coin", 4, "jockey_special")
        self.name = units.Stepper("name", 7)
    ...
    def reset(self):
        super(JockeySpecial, self).reset()
...
game = JockeySpecial(machine_type='pdb')
game.reset()
game.run_loop()
