...
from multi_races.graphics.jockey_club_41 import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.jockey_club_41.display, param=self)

    def sw_flag_active(self, sw):
		if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
			self.game.end_run_loop()
			del self.game.proc
			os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh jockey_club_41")
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.jockey_club_41.display(self)
        self.tilt_actions()
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.jockey_club_41.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.jockey_club_41.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.jockey_club_41.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class JockeyClub41(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(JockeyClub41, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "jockey_club_41", "continuous")
...
game = JockeyClub41(machine_type='pdb')
game.reset()
game.run_loop()
