...
from multi_races.graphics.winning_ticket import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.winning_ticket.display, param=self)
    ...
    def regular_play(self):
    	os.system("sshpass -p raspberry ssh pi@10.0.0.52 /home/pi/sc.sh %s &" % ("winning_ticket_ok"))
        ...
        if self.game.start.status == False:
            ...
            if self.game.parlay.position == 5:
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.winning_ticket.display(self)
        self.tilt_actions()
        self.replay_step_down(self.game.replays)

    def tilt_actions(self):
        os.system("sshpass -p raspberry ssh pi@10.0.0.52 /home/pi/sc.sh %s &" % ("winning_ticket"))
        self.game.coils.payout.disable()
        self.game.payout_number = 0
        self.game.start.disengage()
    ...
    def sw_flag_active(self, sw):
        if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
            ...
            del self.game.proc
            os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh winning_ticket")
    ...
    def find_winner(self, selection, area=None):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if self.game.switches.feature.is_active():
                if self.game.parlay.position >= 5:
                    self.find_winner(self.game.selection, "win")
                else:
                    self.find_winner(selection, "purse")
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.winning_ticket.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.winning_ticket.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class WinningTicket(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(WinningTicket, self).__init__(machine_type)
        ...
        self.coin = units.Stepper("coin", 6)
        self.parlay = units.Stepper("parlay", 5)
    ...
    def reset(self):
        super(WinningTicket, self).reset()
...
game = WinningTicket(machine_type='pdb')
game.reset()
game.run_loop()
