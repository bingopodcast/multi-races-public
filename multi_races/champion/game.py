...
from multi_races.graphics.champion import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.champion.display(self)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.champion.reel1, graphics.champion.reel10, graphics.champion.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.champion.reel1, graphics.champion.reel10, graphics.champion.reel100)
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.champion.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.champion.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.champion.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.champion.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Champion(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(Champion, self).__init__(machine_type)
        ...
        self.coin = units.Stepper("coin", 6, "champion", "continuous")
...
game = Champion(machine_type='pdb')
game.reset()
game.run_loop()
