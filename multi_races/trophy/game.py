...
from multi_races.graphics.trophy import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_flag_active(self, sw):
        if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
            ...
            del self.game.proc
            os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh trophy")
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.trophy.reel1, graphics.trophy.reel10, graphics.trophy.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.trophy.reel1, graphics.trophy.reel10, graphics.trophy.reel100)
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.trophy.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.trophy.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.trophy.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.trophy.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Trophy(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(Trophy, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "trophy", "continuous")
