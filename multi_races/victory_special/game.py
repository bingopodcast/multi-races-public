...
from multi_races.graphics.victory_special import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)

    def sw_flag_active(self, sw):
		if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
			self.game.end_run_loop()
			del self.game.proc
			os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh victory_special")
    ...
    def regular_play(self):
        ...
        if self.game.start.status == False:
            ...
            if self.game.daily_double.status == True:
                if self.game.daily_double_win1.status == True:
                    self.game.daily_double.disengage()
                    if self.game.daily_double_win2.status == True:
                        self.game.daily_double_win2.disengage()
                        self.game.daily_double_win1.disengage()
                else:
                    self.game.daily_double_win1.disengage()
                    self.game.daily_double.disengage()
                    self.game.dd1 = None
                    self.game.dd2 = None
                    self.game.horse1.disengage()
                    self.game.horse2.disengage()
                    self.game.horse3.disengage()
                    self.game.horse4.disengage()
                    self.game.horse5.disengage()
                    self.game.horse6.disengage()
                    self.game.horse7.disengage()
            else:
                self.game.dd1 = None
                self.game.dd2 = None
                self.game.horse1.disengage()
                ...
                self.game.daily_double_win1.disengage()
                self.game.daily_double_win2.disengage()
                self.game.daily_double_win1_number = None
    ...
    def sw_purse1_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.search("purse")
            if self.game.dd1 == 1:
                self.game.horse1.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)
    ...
    def sw_purse2_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.search("purse")
            if self.game.dd1 == 2:
                self.game.horse2.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)
    ...
    def sw_purse3_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.search("purse")
            if self.game.dd1 == 3:
                self.game.horse3.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)
    ...
    def sw_purse4_active_for_40ms(self, sw):
        if self.game.tilt.status == False and self.game.start.status == False:
            self.search("purse")
            if self.game.dd1 == 4:
                self.game.horse4.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)
    ...
    def sw_win5_active_for_40ms(self, sw):
	    ...
	    if self.game.daily_double_win1.status == True:
                if self.game.dd2 == 5:
                    self.game.horse5.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)
    ...
    def sw_win6_active_for_40ms(self, sw):
	    ...
	    if self.game.daily_double_win1.status == True:
                if self.game.dd2 == 6:
                    self.game.horse6.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)
    ...
    def sw_win7_active_for_40ms(self, sw):
	    ...
	    if self.game.daily_double_win1.status == True:
                if self.game.dd2 == 7:
                    self.game.horse7.engage(self.game)
            self.delay(name="display", delay=0.1, handler=graphics.victory_special.display, param=self)
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.victory_special.display(self)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.victory_special.reel1, graphics.victory_special.reel10, graphics.victory_special.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.victory_special.reel1, graphics.victory_special.reel10, graphics.victory_special.reel100)
    ...
    def search(self, area):
        ...
        if self.game.start.status == False:
            ...
            if self.game.daily_double.status == True:
                if self.game.daily_double_win1.status == False:
                    sw = "self.game.switches.purse%s.is_active()" % (dd1)
                    if eval(sw):
                        self.game.daily_double_win1.engage(self.game)
                        self.game.daily_double_win1_number = dd1
                    sw = "self.game.switches.show%s.is_active()" % (dd1)
                    if eval(sw):
                        self.game.daily_double_win1.engage(self.game)
                        self.game.daily_double_win1_number = dd1
                else:
                    sw = "self.game.switches.win%s.is_active()" % (dd2)
                    if eval(sw):
                        self.game.daily_double_win2.engage(self.game)
                        self.game.daily_double_win2_number = dd2
            else:
                if dd2 != None and dd1 != None:
                    sw = "self.game.switches.win%s.is_active()" % (dd2)
                    if eval(sw):
                        if dd2 != self.game.daily_double_win1_number:
                            self.game.daily_double_win2.engage(self.game)
                            self.find_winner("dd2", dd2)
    ...
    def find_winner(self, selection, area=None):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if area == "feature" and self.game.pennant.status == True:
                ...
                self.winner_replay_step_up(40 * m)
            if selection == "dd2" or selection == "dd1" and self.game.daily_double_win2.status == True:
                m = self.get_multiplier()
                self.game.search_index.engage(self.game)
                self.winner_replay_step_up(20 * m)
            if area == "purse":
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.victory_special.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.victory_special.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.victory_special.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class VictorySpecial(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(VictorySpecial, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "victory_special", "continuous")
        ...
        self.daily_double_win1 = units.Relay("daily_double_win1")
        self.daily_double_win2 = units.Relay("daily_double_win2")
        self.horse1 = units.Relay("horse1")
        self.horse2 = units.Relay("horse2")
        self.horse3 = units.Relay("horse3")
        self.horse4 = units.Relay("horse4")
        self.horse5 = units.Relay("horse5")
        self.horse6 = units.Relay("horse6")
        self.horse7 = units.Relay("horse7")

        self.daily_double_win1_number = None
    ...
    def reset(self):
        super(VictorySpecial, self).reset()
...
game = VictorySpecial(machine_type='pdb')
game.reset()
game.run_loop()
