...
from multi_races.graphics.victory_derby import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_flag_active(self, sw):
		if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
			...
			del self.game.proc
			os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh victory_derby")
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.victory_derby.display(self)
        self.tilt_actions()
    ...
    def find_winner(self, selection, area=None):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if selection == "dd2" or selection == "dd1" and self.game.daily_double_win2.status == True:
                m = self.get_multiplier()
                self.game.search_index.engage(self.game)
                self.winner_replay_step_up(20 * m, True)
            if area == "purse":
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.victory_derby.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.victory_derby.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.victory_derby.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class VictoryDerby(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(VictoryDerby, self).__init__(machine_type)
...
game = VictoryDerby(machine_type='pdb')
game.reset()
game.run_loop()
