...
from multi_races.graphics.thorobred import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.thorobred.display, param=self)

    def sw_enter_active(self, sw):
        if self.game.switches.startButton.is_active() and self.game.switches.feature.is_active():
            self.game.end_run_loop()
            os.system("/home/nbaldridge/proc/multi-races/start_game.sh thorobred")
    ...
    def regular_play(self):
        ...
        if self.game.coin.position == 40:
            self.game.selection1 = 1
            self.game.selection2 = 2
            self.game.selection3 = 3
            self.game.selection4 = 4
            self.game.selection5 = 5
            self.game.selection6 = 6
            self.game.selection7 = 7
        self.scan_all()
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.thorobred.display(self)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.thorobred.reel1, graphics.thorobred.reel10, graphics.thorobred.reel100)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.thorobred.reel1, graphics.thorobred.reel10, graphics.thorobred.reel100)
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.thorobred.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.thorobred.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.thorobred.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.thorobred.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Thorobred(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(Thorobred, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "thorobred", "continuous")
        ...
        self.coin = units.Stepper("coin", 40)
        r = random.randint(0,39)
        self.coin.position = r
        self.name = units.Stepper("name", 4)
...
game = Thorobred(machine_type='pdb')
game.reset()
game.run_loop()
