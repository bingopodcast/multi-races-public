...
from multi_races.graphics.santa_anita import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_flag_active(self, sw):
		if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
			...
			del self.game.proc
			os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh santa_anita")
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.santa_anita.display(self)
        self.tilt_actions()
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.santa_anita.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.santa_anita.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.santa_anita.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class SantaAnita(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(SantaAnita, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "santa_anita", "continuous")
...
game = SantaAnita(machine_type='pdb')
game.reset()
game.run_loop()
