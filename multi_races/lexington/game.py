...
from multi_races.graphics.lexington import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.lexington.display, param=self)

    def sw_flag_active(self, sw):
        if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
            self.game.end_run_loop()
            del self.game.proc
            os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh lexington")
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.lexington.display(self)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.lexington.reel1, graphics.lexington.reel10, graphics.lexington.reel100)
    ...
    def sw_leftcap_active(self, sw):
        if self.game.left.status == True:
            self.winner_replay_step_up(4, True)
            self.game.name.step()
            self.game.coils.four_hundred.pulse()
            if self.game.name.position == 5:
                self.winner_replay_step_up(40, True)
        self.delay(name="display", delay=0.1, handler=graphics.lexington.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.lexington.reel1, graphics.lexington.reel10, graphics.lexington.reel100)
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lexington.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lexington.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lexington.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.lexington.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def check_reflex(self):
        if self.game.reflex.connected_rivet() == 5:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("lexington") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 4 and self.game.mixer1.connected_rivet("lexington") not in [2,3,4,5,7,9,10,12,15,17,19,20,21,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("lexington") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 3 and self.game.mixer1.connected_rivet("lexington") not in [2,5,7,9,12,15,19,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("lexington") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 2 and self.game.mixer1.connected_rivet("lexington") not in [5,9,12,15,19,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("lexington") not in [3,10,14,20]:
                return 1
        elif self.game.reflex.connected_rivet() == 1 and self.game.mixer1.connected_rivet("lexington") not in [5,9,15,23]:
            if self.game.abcd.status == False or self.game.mixer1.connected_rivet("lexington") not in [3,10,14,20]:
                return 1
        else:
            return 0
...
class Lexington(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(Lexington, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "lexington", "continuous")
        ...
        self.coin = units.Stepper("coin", 6, "lexington", "continuous")
...
game = Lexington(machine_type='pdb')
game.reset()
game.run_loop()
