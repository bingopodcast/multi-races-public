...
#from multi_races.graphics.photo_finish import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.photo_finish.display, param=self)

    def sw_flag_active(self, sw):
		if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
			self.game.end_run_loop()
			del self.game.proc
			os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh photo_finish")
    ...
    #        self.game.end_run_loop()
    #        os.system("/home/nbaldridge/proc/multi-races/start_game.sh photo_finish")
    ...
    def regular_play(self):
        ...
        if self.game.start.status == False:
            ...
            self.game.purse_place_win.disengage()
            self.game.show_place_win.disengage()
            self.game.three.disengage()
            self.game.four.disengage()
            self.game.five.disengage()
            self.game.show.disengage()
            self.game.place.disengage()
            self.game.win.disengage()
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.photo_finish.display(self)
        self.tilt_actions()
    ...
    def search(self, area):
        if self.game.start.status == False:
            ...
            if self.game.pennant.status == True and self.game.switches.feature.is_active():
                ...
                if self.game.show.status == True:
                    if area == "show":
                        self.find_winner(selection, area)
                if self.game.place.status == True:
                    if area == "place":
                        self.find_winner(selection, area)
                if self.game.win.status == True:
                    if area == "win":
                        self.find_winner(selection, area)
    ...
    def find_winner(self, selection, area):
        if self.game.search_index.status == False and self.game.replays < 899:
            ...
            if self.game.purse_place_win.status == True:
                place = win
                purse = win
            if self.game.show_place_win.status == True:
                show = win
                place = win
            if area == "feature" and self.game.pennant.status == True:
                ...
                if self.game.odds.position in [8,9,10]:
                    if self.game.replay_counter.position < 160:
                        ...
                        self.winner_replay_step_up(160 - self.game.replay_counter.position)
            if area == "purse":
                if self.game.purse_place_win.status == False:
                ...
                if self.game.show_place_win.status == False:
                ...
                if self.game.show_place_win.status == False or self.game.purse_place_win.status == False:
                    if self.game.replay_counter.position < place:
                        self.game.search_index.engage(self.game)
                        self.winner_replay_step_up(place - self.game.replay_counter.position)
                if self.game.show_place_win.status == True or self.game.purse_place_win.status == True:
                    if self.game.replay_counter.position < win:
                        self.game.search_index.engage(self.game)
                        self.winner_replay_step_up(win - self.game.replay_counter.position)
            else:
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.photo_finish.star_animation([self, start + num])
    ...
    def check_extra_step(self):
        ...
        def step_odds(self, number):
            ...
            self.delay(name="display", delay=0.1, handler=graphics.photo_finish.display, param=self)
            number -= 1
            self.delay(name="step_odds", delay=0.3, handler=self.step_odds, param=number)
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.photo_finish.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.photo_finish.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.photo_finish.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def clover_probability(self):
        if self.check_reflex() == 1:
            ...
            if self.game.spotting2.position in [13,22,33]:
                self.game.show.engage(self.game)
            if self.game.spotting2.position in [5,11,27]:
                self.game.place.engage(self.game)
            if self.game.spotting2.position in [9,14,32]:
                self.game.win.engage(self.game)
            if self.game.spotting2.position in [1,19,21]:
                self.game.purse_place_win.engage(self.game)
            if self.game.spotting2.position in [2,8,20]:
                self.game.show_place_win.engage(self.game)
...
class PhotoFinish(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(PhotoFinish, self).__init__(machine_type)
        ...
        self.coin = units.Stepper("coin", 1, "photo_finish", "continuous")
        self.coin6 = units.Stepper("coin6", 5, "photo_finish", "continuous")
        ...
        self.purse_place_win = units.Relay("purse_place_win")
        self.show_place_win = units.Relay("show_place_win")
        self.three = units.Relay("three")
        self.four = units.Relay("four")
        self.five = units.Relay("five")
        self.fan = units.Relay("fan")
        self.show = units.Relay("show")
        self.place = units.Relay("place")
        self.win = units.Relay("win")
...
game = PhotoFinish(machine_type='pdb')
game.reset()
game.run_loop()
