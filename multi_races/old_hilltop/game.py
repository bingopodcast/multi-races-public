...
#from multi_races.graphics.old_hilltop import *
...
class OneBall(procgame.game.Mode):
    ...
    #        self.game.end_run_loop()
    #        os.system("/home/nbaldridge/proc/multi-races/start_game.sh old_hilltop")
        ...
        if self.game.start.status == False:
            ...
            if self.game.feature_lite.status == True and self.game.feature_lite_won.status == True:
                ...
                if self.game.feature_lite.status == False:
                    self.game.pennant.disengage()
            if self.game.parlay.position >= 6:
                if self.game.super_feature.status == True:
                    self.game.super_feature.disengage()
                    self.game.parlay.reset()
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.old_hilltop.display(self)
        self.tilt_actions()
    ...
    def search(self, area):
        if self.game.start.status == False:
            ...
            if self.game.pennant.status == True and self.game.switches.feature.is_active():
                ...
                if self.game.parlay.position >= 6:
                    self.game.super_feature.engage(self.game)
            else:
                ...
                if self.game.super_feature.status == True:
                    ...
                    if self.game.odds.position in [6,7]:
                        if self.game.replay_counter.position < 160:
                            ...
                            self.winner_replay_step_up(160 - self.game.replay_counter.position)
                    if self.game.odds.position in [8,9,10]:
                        if self.game.replay_counter.position < 320:
                            self.game.search_index.engage(self.game)
                            self.winner_replay_step_up(320 - self.game.replay_counter.position)
            else:
                if area == "feature":
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.old_hilltop.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.old_hilltop.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.old_hilltop.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.old_hilltop.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def clover_probability(self):
        if self.check_reflex() == 1:
            ...
            if self.game.spotting2.position in [15,23,40,48]:
                ...
                self.game.pennant.engage(self.game)
            if self.game.spotting2.position in [16,41,8,34]:
                self.game.parlay.step()
                self.game.coils.four_hundred.pulse()
...
class OldHilltop(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(OldHilltop, self).__init__(machine_type)
        ...
        self.coin = units.Stepper("coin", 1, "old_hilltop", "continuous")
        self.coin6 = units.Stepper("coin6", 5, "old_hilltop", "continuous")
        ...
        self.parlay = units.Stepper("parlay", 6)
        r = random.randint(0,5)
        self.parlay.position = r
        ...
        self.super_feature = units.Relay("super_feature")
...
game = OldHilltop(machine_type='pdb')
game.reset()
game.run_loop()
