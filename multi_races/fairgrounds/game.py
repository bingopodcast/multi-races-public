...
from multi_races.graphics.fairgrounds import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.fairgrounds.display, param=self)
    ...
    def regular_play(self):
    	os.system("sshpass -p raspberry ssh pi@10.0.0.52 /home/pi/sc.sh %s &" % ("fairgrounds_ok"))
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.fairgrounds.display(self)
        self.tilt_actions()
        self.replay_step_down(self.game.replays)

    def tilt_actions(self):
	os.system("sshpass -p raspberry ssh pi@10.0.0.52 /home/pi/sc.sh %s &" % ("fairgrounds"))
        self.game.coils.payout.disable()
        self.game.payout_number = 0
        self.game.start.disengage()
	...
	if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
	    ...
	    del self.game.proc
	    os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh fairgrounds")
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.fairgrounds.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.fairgrounds.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Fairgrounds(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(Fairgrounds, self).__init__(machine_type)
...
game = Fairgrounds(machine_type='pdb')
game.reset()
game.run_loop()
