...
from multi_races.graphics.thistledowns import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.thistledowns.display, param=self)
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.thistledowns.display(self)
        self.tilt_actions()
	...
	if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
	    ...
	    del self.game.proc
	    os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh thistledowns")
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.thistledowns.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.thistledowns.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class Thistledowns(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(Thistledowns, self).__init__(machine_type)
...
game = Thistledowns(machine_type='pdb')
game.reset()
game.run_loop()
