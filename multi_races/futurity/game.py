...
class OneBall(procgame.game.Mode):
    ...
    def regular_play(self):
        ...
        if self.game.start.status == False:
            ...
            if self.game.entry_won.status == True:
                if self.game.number_three.status == True:
                    self.game.hold3.engage(self.game)
                    self.game.number_three.disengage()
                    self.game.selection.append(3)
                if self.game.number_four.status == True:
                    self.game.hold4.engage(self.game)
                    self.game.number_four.disengage()
                    self.game.selection.append(4)
                if self.game.number_five.status == True:
                    self.game.hold5.engage(self.game)
                    self.game.number_five.disengage()
	...
	if self.game.switches.star.is_active() and self.game.switches.horseshoe.is_active():
	    ...
	    del self.game.proc
	    os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh futurity")
	else:
	    if self.game.start.status == True:
        ...
        if self.game.spotting2.position == 2:
            if self.game.trophy.status == False:
                ...
                if self.game.number_four.status == False:
                    self.game.number_four.engage(self.game)
        if self.game.spotting2.position == 3:
            if self.game.entry.status == True:
                if self.game.number_three.status == False:
                    self.game.number_three.engage(self.game)
        if self.game.spotting2.position == 4:
            if self.game.entry.status == True:
                if self.game.number_five.status == False:
                    self.game.number_five.engage(self.game)
    ...
    def scan_futurity(self):
        self.check_futurity()
        self.delay(name="display", delay=0.1, handler=graphics.futurity.display, param=self)

    def animate_futurity_scan(self, args):
        start = args[0]
        diff = args[1]
        num = args[2]
        if start + num >= 50:
        ...
        if diff >= 0:
            num = num + 1
            graphics.futurity.special_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
            self.delay(name="futurity_animation", delay=0.05, handler=self.animate_futurity_scan, param=args)
        else:
            self.cancel_delayed(name="futurity_animation")
            self.delay(name="display", delay=0.1, handler=graphics.futurity.display, param=self)
            self.scan_futurity()
    ...
    def scan_all(self, begin1, begin2, end1, end2):
        if self.game.all_advantages.status == True or self.game.star.status == True:
            s = self.animate_star_scan([begin1, self.game.spotting.position,end1])
        ...
        if self.game.all_advantages.status == True or self.game.horseshoe.status == True:
            if self.game.fan.status == False:
                self.game.selection = []
                h = self.animate_selection_scan([begin1,self.game.spotting.position,1])
            self.reset_clover()
        if self.game.all_advantages.status == True or self.game.clover.status == True:
            i = self.animate_clover_scan([begin1,self.game.spotting.position,1])
        if self.game.all_advantages.status == True or self.game.feature.status == True:
            f = self.animate_feature_scan([begin1,self.game.spotting.position,1])
        j = self.animate_futurity_scan([begin1, self.game.spotting.position,end1])
...
class Futurity(procgame.game.BasicGame):
    ...
    def __init__(self, machine_type):
        ...
        self.number_three = units.Relay("number_three")
        self.number_four = units.Relay("number_four")
        self.number_five = units.Relay("number_five")
