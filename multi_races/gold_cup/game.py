...
from multi_races.graphics.gold_cup import *
...
class OneBall(procgame.game.Mode):
    ...
    def sw_coin_active(self, sw):
        ...
        self.regular_play()
        self.delay(name="display", delay=0.1, handler=graphics.gold_cup.display, param=self)
    ...
    def sw_replayReset_active(self, sw):
        ...
        graphics.gold_cup.display(self)
        self.tilt_actions()
    ...
    def replay_step_down(self, number=0):
        if number > 0:
            if number > 1:
                self.game.replays -= 1
                graphics.replay_step_down(self.game.replays, graphics.gold_cup.reel1, graphics.gold_cup.reel10, graphics.gold_cup.reel100)
    ...
    def sw_leftcap_active(self, sw):
        ...
        self.delay(name="display", delay=0.1, handler=graphics.trophy.display, param=self)
    ...
    def replay_step_up(self):
        if self.game.replays < 899:
            self.game.replays += 1
            graphics.replay_step_up(self.game.replays, graphics.gold_cup.reel1, graphics.gold_cup.reel10, graphics.gold_cup.reel100)
    ...
    def sw_horseshoe_active(self, sw):
        if self.game.switches.clover.is_active() and self.game.switches.flag.is_active():
           ...
           os.system("/home/nbaldridge/proc/multi-races/multi_races/start_game.sh gold_cup")
        else:
    ...
    def check_hold(self):
        if self.game.four_hundred.position % 3 == 0.15:
            if self.game.spotting.position in [33,34]:
                if self.game.four_hundred.position % 2 == 0:
    ...
    def animate_star_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.gold_cup.star_animation([self, start + num])
    ...
    def animate_selection_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.gold_cup.selection_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_feature_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.gold_cup.feature_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
    ...
    def animate_clover_scan(self, args):
        ...
        if diff >= 0:
            num = num + 1
            graphics.gold_cup.clover_animation([self, start + num])
            self.cancel_delayed(name="display")
            diff = diff - 1
            args = [start,diff,num]
...
class GoldCup(procgame.game.BasicGame):
    """ Turf King was one of the last one balls made by Bally """
    def __init__(self, machine_type):
        super(GoldCup, self).__init__(machine_type)
        ...
        self.four_hundred = units.Stepper("four_hundred", 400, "gold_cup", "continuous")
